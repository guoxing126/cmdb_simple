...
from ..service_config import server as server_config
from .base import BasiceService

class ServerService(BasiceService):
    ...
    def fetch(self):
        ...
        total_item_count = models.Server.objects.filter(self.conditions()).count()
        page_obj = Pagination(current_page, total_item_count, per_page_count=5)

        server_list = models.Server.objects.filter(self.conditions()).values(*self.values())[page_obj.start:page_obj.end]
